...
import { addSchema } from "./addproductschemes";
...
async function submitRegister(data) {
  ...
  try {
    ...
    // Convert data to match the schema expected by the server
  const formattedData = {
  ...data,
  districts: arrayFields.districts,
  sowing_month: arrayFields.sowing_month,
  season: arrayFields.season,
  water: arrayFields.water,
  soil: arrayFields.soil,
  heat: arrayFields.heat,
  climate: arrayFields.climate,
...
stock: v.stock ? Number(v.stock) : 0
    ...
    const { image, weight, price, stock, ...rest } = formattedData;
    ...
    if (!response.ok) {
      ...
      const errorMessages = result.fields 
        ? Object.entries(result.fields).map(([key, msgs]) => `${key}: ${msgs.join(', ')}`).join('\n')
        : result.error;
      toast.error(errorMessages || result.message || "Failed to add product");
      return;
    }
...
<div className="flex justify-between px-4 items-center ">
    ...
    <div>
   <h3 className="text-2xl text-emerald-400 font-semibold text-center">Add New Seeds</h3>
    </div>
...
{/* category */}
<div className="flex flex-col relative">
    <label className="text-xs font-semibold  text-gray-700  " htmlFor="category">Category Seeds</label>
...
<div className="flex flex-col col-span-1 md:col-span-3  " >
    ...
    <input       {...register("image", {  required: "Image is required" })} className="py-3 mt-1 px-5 rounded-lg  outline-none border border-gray-300 focus:ring-2 focus:ring-gray-500" required type="file" id="image" placeholder="Enter the image URL" />
